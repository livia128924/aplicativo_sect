      ...
      (tx) => {
        tx.executeSql(
          ...
          (tx, results) => {
            ...
            setItem_se_duf_situacao_imovel(temp);
          }
        );
        tx.executeSql(
          "select * from aux_tempo_ocupacao",
          [],
          (tx, results) => {
            var temp = [];
            for (let i = 0; i < results.rows.length; ++i) {
    ...
    <>
      <View style={styles.form8}>
        <View style={styles.rect2}>
          <Text style={styles.titulo}>OCUPACAO DO IMOVEL</Text>
        </View>
        <View style={styles.municipio}>
          <Text>Situacao do Imovel</Text>
        </View>
        <View style={{ alignItems: "center", }}>
        <DropDownPicker
          style={styles.drop_down_Style}
          zIndex={open_se_duf_situacao_imovel ? 9999 : 0}
          open={open_se_duf_situacao_imovel}
          value={parseInt(valor_se_duf_situacao_imovel)}
          items={item_se_duf_situacao_imovel}
          setOpen={setOpen_se_duf_situacao_imovel}
          setValue={setValor_se_duf_situacao_imovel}
          setItems={setItem_se_duf_situacao_imovel}
          onChangeValue={() =>
            onPressTitle(
              "se_duf",
              "se_duf_situacao_imovel",
              valor_se_duf_situacao_imovel,
              sync
              )
            }
            listMode="SCROLLVIEW"
        ...
        <DropDownPicker
          style={styles.drop_down_Style}
          zIndex={open_se_duf_tempo_ocupacao ? 9999 : 0}
          open={open_se_duf_tempo_ocupacao}
          value={parseInt(valor_se_duf_tempo_ocupacao)}
          items={item_se_duf_tempo_ocupacao}
