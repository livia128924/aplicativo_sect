...
const Step6 = (props) => {
                ...
                tx.executeSql(
                    "select * from " + tabela + " where se_ruj_cod_processo = '" + cod_processo + "'", [], (tx, results) => {
                        ...
                        for (let i = 0; i < results.rows.length; ++i) {
                            ...
                            setValorSe_ruj_responsabilidade_social(results.rows.item(i).s_ruj_responsabilidade_social);
                            setValorSe_ruj_formacao_atuacao(results.rows.item(i).se_ruj_formacao_atuacao);
                            setValorSe_ruj_investimento_financeiro(results.rows.item(i).se_ruj_investimento_financeiro);

                            // x = results.rows.item(i).se_ruj_bens_moveis;
                            // valor_checked(x.split(','));

                            // bens_imoveis = results.rows.item(i).Se_ruj_bens_imoveis;
  ...
  async function loadStep6() {
    db.transaction((tx) => {
      tx.executeSql(
        "select * from aux_bens_moveis", [], (tx, results) => {
      ...
      // );
      tx.executeSql(
        "select * from aux_formacao_atuacao", [], (tx, results) => {
          //var len = results.rows.length, i;
          var temp = [];
          //console.log(len);
          for (let i = 0; i < results.rows.length; ++i) {
      ...
      tx.executeSql(
        "select * from aux_investimento_financeiro", [], (tx, results) => {
          //var len = results.rows.length, i;
          var temp = [];
          //console.log(len);
          for (let i = 0; i < results.rows.length; ++i) {
...
const handleChange = async (index) => {
  ...
  let _valores = [...se_ruj_bens_moveis];
  ...
  return setSe_ruj_bens_moveis(_valores);

};
const handleChange_bens_imoveis = async (index) => {

  //console.log(index);
  let value = index.value;
  let checked = index.isChecked;

  let _valores = [...se_ruj_bens_imoveis];
  ...
  //console.log(ischecado);
  return setSe_ruj_bens_imoveis(_valores);
...
function muda() {
  ...
  se_ruj_bens_moveis.filter(value => value.isChecked === true).map((item) => {
...
function muda_bens_imoveis() {
  ...
  se_ruj_bens_imoveis.filter(value => value.isChecked === true).map((item) => {
      str_valores.push(item.value);
  });
  //console.log(ischecado);
  return str_valores.join(",");
  ...
  return (
    <>
      <View style={styles.form9}>
          ...
          {[...se_ruj_bens_moveis].map((item, index) => (
            ...
            >
              <Checkbox
                ...
                onValueChange={() => {
                  handleChange(item); onPressTitle("se_ruj", "se_ruj_bens_moveis", muda(), sync)
                }}
              />
        ...
        {[...se_ruj_bens_imoveis].map((item, index) => (
          ...
          <Checkbox
             ...
             value={item.isChecked}
            onValueChange={() => {
              handleChange_bens_imoveis(item); onPressTitle("se_ruj", "se_ruj_bens_imoveis", muda_bens_imoveis(), sync)
            }}
          />
          <Text>{item.label}</Text>
          </View>
      ...
      <View style={styles.form10}>
        ...
        <Text style={styles.titleStyle}>Possui programa de responsabilidade social</Text>
      ...
      <View style={{zIndex:9999}}>
        <DropDownPicker
          ...
          onChangeValue={() => onPressTitle("se_ruj", "Se_ruj_destino_dejetos", valorSe_ruj_responsabilidade_social, sync)}
          listMode="SCROLLVIEW"
        ...
        <DropDownPicker
          ...
          onChangeValue={() => onPressTitle("se_ruj", "Se_ruj_destino_dejetos", valorSe_ruj_formacao_atuacao, sync)}
          listMode="SCROLLVIEW"
          placeholder="Selecione"
        />
        ...
        <DropDownPicker
          ...
          onChangeValue={() => onPressTitle("se_ruj", "Se_ruj_destino_dejetos", valorSe_ruj_investimento_financeiro, sync)}
          listMode="SCROLLVIEW"
          placeholder="Selecione"
        />
