...
const Sincronizacao = ({ navigation }) => {
  ...
  async function onPress(codigo) {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        `SELECT * FROM log where situacao = '1' and cod_processo= '${codigo}' `,
        [],
        (tx, results) => {
          ...
          for (let i = 0; i < results.rows.length; ++i) {
            ...
            //console.log(nome);
            api_meta
              .post("/_apps/app_teste/update/index.php", {
                processo,
                valor,
                tabela,
                campo,
              ...
              .then(function (response) {
                ...
                if (status === "OK") {
                  ...
                  db.transaction((tx) => {
                    tx.executeSql(
                      `UPDATE log set situacao = '2' where cod_processo = '${codigo}'`,
                      [],
                      (tx, results) => {}
                    );
                  });
          ...
          <View style={styles.item}>
            {[...flatListItems].map((item, index) => (
              <View style={styles.map} key={item.pr_codigo}>
                <TouchableOpacity
                  onPress={() =>
                    onPressProcesso(item.pr_codigo, item.pr_numero_processo)
                  }
                  style={styles.title}
                >
                  <Text>{item.pr_numero_processo}</Text>
                </TouchableOpacity>
                <Text style={styles.title}>{item.count}</Text>
                <Text style={styles.title}>{item.enviado}</Text>
                <Button
                  onPress={() => onPress(item.pr_codigo)}
                  title=""
                  //type="clear"
                  icon={<Icon name="database-sync" size={15} color="white" />}
                />
              </View>
            ))}
          </View>
